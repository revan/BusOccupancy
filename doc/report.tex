...
Two students worked on each approach, and we planned to combine results into a single prediction method.
    ...
    In order to better utilize this data, we first parsed the log files for information relevant to our goal.
    The log file contained many details we didn't need -- things like the type of packet, or the antenna used.
    The first step in creating the graphs was to sanitize the data; we couldn't leave all these MAC addresses since they could potentially be used to identify people.
    We parsed the log files, assigning each address an index number starting from zero.
    Each line was converted into an Python Dict holding the four potential addresses, the packet's strength, and what time it was sent at.
    Some packets did not have a strength field -- these were assigned a strength of zero, which can easily be filtered out later, since this is not a strength which could potentially appear naturally.
    Lines which were identified as routers broadcasting their existance were removed, and the address identified as a router was flagged in case they might need to be filtered later.
    At the end of the parsing process, a JSON output is produced.
    JSON is a open standard data interchange format.
    Using JSON made utilizing the data much easier when it came to actually producing the graphs -- most languages, Python included, have a standard library implementation for JSON, eliminating the effort spent in re-parsing the data.\\

    For the actual construction of the graphs, we needed two additional libraries -- Numpy and Pandas.
    ...
    First, these JSON files were re-read and passed into a Pandas DataFrame.
    ...
    After the data has been filtered, it can be handed off to six different helper scripts; one for each graph type.\\
